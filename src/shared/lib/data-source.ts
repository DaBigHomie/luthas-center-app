 ...
 *
 * Cover resolution:
 *   resolveCoverUrl(entity, type) → per-item branded SVG from public/covers/ when the
 *   real image is missing, falling back to /placeholder-cover.svg only if the SVG file
 *   is absent on disk.
 *
 * Curation:
 *   - listPublishedCourses / getCourseWithCurriculum / sitemap exclude courses with zero
 *     lessons AND zero quizzes (hasCurriculum = false). Pending courses are always excluded.
 *   - listPosts / countPosts / sitemap exclude posts whose text content (tags stripped)
 *     is < 200 chars (stub posts). Stubs remain retrievable by slug (getPostBySlug).
 */
...
// ---------------------------------------------------------------------------
// Cover URL resolution — per-item branded SVG with media fallback
// ---------------------------------------------------------------------------

/**
 * Resolve the best cover URL for a course or post entity.
 *
 * Priority:
 *   1. If media is available and resolves to a real path (not placeholder) → use it.
 *   2. Otherwise use the per-item branded SVG at /covers/<type>-<slug>.svg if the
 *      file exists on disk (generated by scripts/gen-covers.mjs).
 *   3. Final fallback: /placeholder-cover.svg.
 *
 * @param media   - MediaRow or null for the entity's featured_image.
 * @param type    - 'course' | 'post'
 * @param slug    - entity slug (used to build the SVG filename)
 */
export function resolveCoverUrl(
  ...
  // Try real media first
  if (media) {
    const resolved = resolveMediaUrl(media)
    if (resolved !== '/placeholder-cover.svg') return resolved
  }

  // Try per-item branded SVG
  const svgRelPath = `/covers/${type}-${slug}.svg`
  const svgDiskPath = path.join(process.cwd(), 'public', svgRelPath)
  if (existsSync(svgDiskPath)) return svgRelPath
...
/**
 * Returns true when a course has at least one lesson or quiz (real curriculum).
 * Uses step_counts from the raw JSON record; falls back to checking course_steps[].
 */
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function courseHasCurriculum(r: Record<string, any>): boolean {
  const sc = r.step_counts as { lessons?: number; quizzes?: number } | null | undefined
  if (sc) {
    return (sc.lessons ?? 0) > 0 || (sc.quizzes ?? 0) > 0
  }
  // Fallback: check course_steps array length
  return Array.isArray(r.course_steps) && r.course_steps.length > 0
}

/** Minimum character count for a post to appear in listings (stub guard). */
const POST_MIN_CHARS = 200

/**
 * Strip HTML tags and return the text length of a post's content.
 */
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function postTextLength(r: Record<string, any>): number {
...
export async function listPublishedCourses(): Promise<CourseSummary[]> {
  ...
  const records = loadJson<RawCourse[]>('courses.json')
  const published = records.filter((r) => r.status === 'publish')
  const withCurriculum = published.filter(courseHasCurriculum)
  const excluded = published.length - withCurriculum.length
  if (excluded > 0) {
    console.log(`[data-source] listPublishedCourses: excluded ${excluded} empty-shell course(s) (no lessons/quizzes) from ${published.length} published`)
  }
...
export async function getCourseBySlug(slug: string): Promise<CourseRow | null> {
  ...
  // Only return courses that have curriculum (empty-shell courses are excluded from public URLs).
  const found = records.find(
    (r) => resolveCourseSlug(r) === slug && r.status === 'publish' && courseHasCurriculum(r),
  )
...
export async function countPosts(categorySlug?: string): Promise<number> {
  ...
  let filtered = records.filter((r) => r.status === 'publish' && postTextLength(r) >= POST_MIN_CHARS)
  if (categorySlug) {
    filtered = filtered.filter(
      (r) =>
        Array.isArray(r.categories) &&
